...
| 章节数 | 覆盖科目 | 更新节奏 | 技术栈 |
|---:|---:|---:|---|
| 7 章持续增加中 | 化学 + 物理 | 日更 | 纯 HTML/CSS/JS，零构建 |
...
| 05 | 机械能守恒 | 2026-08-27 | 物理 | 只有重力/弹力做功，机械能不变 |
| 06 | 天体运动 | 2026-08-27 | 物理 | 绕同一天体：高轨低速大周期 |
| 07 | 宇宙速度 | 2026-08-27 | 物理 | 近地环绕速度 = 第一宇宙速度 ≈ 7.9 km/s |
...
```
.
├── index.html      # 单页：Hero、今日精选、知识点目录挂载点、章节挂载点、方法论、CTA
├── content.js      # ★ 全部章节数据——加内容只改这一个文件
├── render.js       # 渲染目录（renderToc → #tocMount）+ 章节卡（renderList → #exploreMount），按分值排序
...
```js
{
  ...
  score: 90                               // 优先级分 1-100（越大越值得优先搞懂），首页按它排序
}
```
