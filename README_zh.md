...
大多数复习资料给你一堆公式让你背。这个项目反着来：每个高考知识点先拆解到**第一性原理**，再用一个**视觉隐喻**重建——导数是加速的车，受力分析是拔河的两边——让「看懂」替你干活，而不是重复刷题。
...
| 章节数 | 覆盖科目 | 更新节奏 | 技术栈 |
|---:|---:|---:|---|
| 8 章持续增加中 | 数学 · 物理 · 化学 | 日更 | 纯 HTML/CSS/JS，零构建 |

| № | 章节 | 科目 | 一句话本质 |
|---|---|---|---|
| 01 | 导数 | 数学 | 导数不是公式——是**正在发生的**变化 |
| 02 | 受力分析 | 物理 | 所有力：先拆解，再合成 |
| 03 | 化学平衡 | 化学 | 平衡不是静止——是**势均力敌** |
| 04 | 三角函数 | 数学 | 圆周运动投影到直尺上 |
| 05 | 物质分类 | 化学 | 键型看构成微粒，不看名字猜 |
| 06 | 分离提纯 | 化学 | 按沸点分离：蒸发、冷凝、收集 |
| 07 | 高分子材料 | 化学 | PLA：大自然能「吃掉」的塑料 |
| 08 | 硫及其化合物 | 化学 | SO₂ 使 KMnO₄ 褪色靠的是**还原性**，不是漂白性 |
